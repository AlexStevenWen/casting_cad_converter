from cad_converter import process_3d_file_unified_batch
import transform_3D_batch 
import pandas as pd
import os
import shutil
from ShapeCastDATA import PdReadShape
from SySPath import ChynWangDatapy , ChynWangPojectpy
import matplotlib.pyplot as plt
import plotly.express as px
import plotly.subplots as sp
from tqdm import tqdm
import numpy as np
import csv
import transform_3D_batch
CW_root = ChynWangDatapy()
CP_root = ChynWangPojectpy()

CAD_source_root = os.path.join(CP_root,"data", "access_raw_CAD")
CAD_input_root = os.path.join(CP_root, "data", "step_file_raw")

processed_table_root = os.path.join(CP_root, "data", "processed_table")
processed_CAD_root = os.path.join(CP_root, "data", "processed_CAD")

# transform_3D_batch.batch_transform_Step_to_Parameter(
#     os.path.join(processed_CAD_root, "step_upright_z_merge", "tree"),
#     os.path.join(processed_CAD_root, "parameter_upright_z_merge_identify", "tree")
# )

transform_3D_batch.batch_rename_and_copy_by_mode(
    os.path.join(processed_CAD_root, "csv_identify_parts"),
    os.path.join(processed_CAD_root, "step_upright_z_parts", "tree"),
    os.path.join(processed_CAD_root, "step_upright_z_parts_identify", "parts"),mode='.step')

transform_3D_batch.batch_transform_Step_to_3D(
    os.path.join(processed_CAD_root, "step_upright_z_parts_identify", "parts"),
    os.path.join(processed_CAD_root, "step_upright_z_parts_identify", "id_parts"),mode='.step',do_align = True
)

# 第 3 行
transform_3D_batch.batch_transform_Step_to_Parameter(
    os.path.join(processed_CAD_root, "step_upright_z_parts_identify", "id_parts"),
    os.path.join(processed_CAD_root, "parameter_upright_z_merge_identify", "blank")
)
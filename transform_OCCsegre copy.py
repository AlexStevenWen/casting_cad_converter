...
def read_main_solid_and_volume(step_path):
    ...
    try:
        shutil.copy2(str(step_path), temp_path)
        reader = STEPControl_Reader()
        status = reader.ReadFile(temp_path)
        
        if status != 1:
            print(f"ERROR: 讀取 STEP 檔案失敗: {step_path.name}")
            return None, 0
        ...
        if not all_solids:
            print(f"ERROR: 在 {step_path.name} 中未找到任何有效的實體 (Solid)")
            return None, 0
        ...
        if main_solid is None or max_volume <= 1e-6:
            print(f"ERROR: 未能在 {step_path.name} 中找到有效的主實體")
            return None, 0

    except Exception as e:
        print(f"ERROR: 解析檔案 {step_path.name} 時發生錯誤: {e}")
        return None, 0
    finally:
        try:
